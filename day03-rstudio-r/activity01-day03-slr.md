...
``` r
# Creating an R object called `int_vect` that contains the values 1 through 9
int_vect <- 1:9
int_vect
```
...
``` r
# Creating an R objected called `char_vect` that contains the letters "A" till"Q"
...
``` r
# Determining how many of the items in `char_vect` occur after the letter "D"
after_D <- char_vect[which(char_vect > "D")]
count_after_D <- length(after_D)
count_after_D
```
